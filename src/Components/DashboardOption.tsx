...
const DashboardOption = ({
  ...
  return (
    <Link
      className={`flex items-center py-3 pl-10 ${
        isActive === name && "bg-primaryOne rounded-l-lg"
      } transition-all cursor-pointer`}
      onClick={() => setIsActive(name)}
      to={name}
    >
      <DashboardOptionLogo isActive={isActive} name={name} />
      <p className={`ml-2 font-semibold ${isActive === name && "text-white"}`}>
        {capitalizeName(name)}
      </p>
    </Link>
  );
    ...
    return (
      ...
      >
        <path
          ...
          fill={isActive === name ? "white" : "#525063"}
        />
      </svg>
    );
